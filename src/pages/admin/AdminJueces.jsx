...
export default function AdminJueces() {
  ...
  const [filtros, setFiltros] = useState({ nombre: "", dni: "", licencia: "" });
  const [filtrosAplicados, setFiltrosAplicados] = useState({ nombre: "", dni: "", licencia: "" });
    ...
    try {
      const res = await api.get("/admin/jueces", {
        params: {
          ...
          licencia: filtrosAplicados.licencia || undefined
        }
      });
  ...
  }, [page, filtrosAplicados.nombre, filtrosAplicados.dni, filtrosAplicados.licencia]);
  ...
  const aplicarBusqueda = () => {
    const next = {
      ...
      licencia: filtros.licencia.trim()
    };

    if (
      next.nombre === filtrosAplicados.nombre &&
      next.dni === filtrosAplicados.dni &&
      next.licencia === filtrosAplicados.licencia
    ) return;
  ...
  const limpiarBusqueda = () => {
    if (
      !filtros.nombre && !filtros.dni && !filtros.licencia &&
      !filtrosAplicados.nombre && !filtrosAplicados.dni && !filtrosAplicados.licencia
    ) return;

    setFiltros({ nombre: "", dni: "", licencia: "" });
    setPage(1);
    setFiltrosAplicados({ nombre: "", dni: "", licencia: "" });
  };
  ...
  const guardar = async () => {
    if (!form.dni.trim() || !form.nombres.trim() || !form.apellidos.trim() || !form.correo.trim() || !form.licencia.trim()) {
      return Swal.fire("Atencion", "Nombre, apellidos, correo y licencia son obligatorios", "warning");
    }

    if (!editingId && !form.contrasena.trim()) {
      return Swal.fire("Atencion", "La contrasena es obligatoria para nuevos registros", "warning");
    }

    if (!adminId) return Swal.fire("Error", "Tu sesion expiro. Vuelve a iniciar sesion.", "error");
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || err.response?.data?.mensaje || "Error al procesar la solicitud";
  ...
  const eliminar = async (idJuez) => {
    const result = await Swal.fire({
      title: "Eliminar juez?",
      text: "Esta accion podria afectar el historial de competencias.",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire("Exito", "Juez removido correctamente.", "success");
        cargar();
      } catch {
  ...
  const inactivarLicencia = async (idJuez) => {
    const result = await Swal.fire({
      ...
      text: "El juez dejara de calificar y solo podra competir.",
      icon: "warning",
      showCancelButton: true,
  ...
  const cambiarEstado = async (idJuez, accion) => {
    try {
      ...
      Swal.fire("Exito", `El juez ha sido ${accion === "aprobar" ? "aprobado" : "rechazado"}`, "success");
      cargar();
    } catch {
      Swal.fire("Error", "Fallo al cambiar el estado de validacion", "error");
    }
  };
  ...
  return (
    <div className="container-fluid px-4 py-4 animate__animated animate__fadeIn">
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4 gap-3">
        <div>
          <h2 className="fw-bold text-dark mb-0"><FaUserTie className="me-2 text-primary" />Gestion de Jueces</h2>
          <p className="text-muted mb-0">Control de credenciales y validacion de licencias tecnicas.</p>
        </div>
        ...
        <div className="card-body p-2">
          <div className="row g-2 align-items-end mb-2">
            ...
            <div className="col-12 col-md-2">
              <label className="form-label small fw-bold mb-1">Licencia</label>
              <input
                type="text"
                className="form-control border-0 shadow-none bg-light"
                placeholder="Licencia..."
                value={filtros.licencia}
                onChange={(e) => setFiltros(prev => ({ ...prev, licencia: e.target.value }))}
                onKeyDown={(e) => { if (e.key === "Enter") aplicarBusqueda(); }}
              />
            </div>
          ...
          <small className="text-muted px-2">Filtra por nombre, DNI o licencia y presiona Enter o Buscar.</small>
        </div>
      </div>
          ...
          <table className="table table-hover align-middle mb-0">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="6" className="text-center text-muted py-5">No se encontraron jueces con los criterios de busqueda</td></tr>
              ) : (
                jueces.map(j => (
                  <tr key={j.idJuez}>
                    <td className="ps-4">
              ...
              <div className="modal-body p-4">
                <div className="row g-3">
                  <div className="col-12">
                    <label className="form-label fw-bold text-muted small">DNI DEL JUEZ *</label>
                    <input
                      ...
                      maxLength={8}
                    />
                    <button className="btn btn-outline-info mt-2" onClick={cargarPorDni}>Cargar</button>
                  </div>
                </div>
              </div>
              ...
              <div className="modal-body p-4">
                <div className="row g-3">
                  ...
                  <div className="col-md-6">
                    ...
                    <input className="form-control rounded-3 bg-light border-0 py-2" value={form.apellidos} onChange={e => setForm({ ...form, apellidos: e.target.value })} placeholder="Ej: Perez Garcia" />
                  </div>
                  <div className="col-12">
                    <label className="form-label fw-bold text-muted small">CORREO ELECTRONICO *</label>
                  ...
                  <div className="col-md-6">
                    <label className="form-label fw-bold text-muted small">TELEFONO (OPCIONAL)</label>
                    <input
                      type="tel"
                      className="form-control rounded-3 bg-light border-0 py-2"
                      value={form.telefono}
                  ...
                  <div className="col-md-6">
                    <label className="form-label fw-bold text-muted small">NRO LICENCIA *</label>
                  ...
                  <div className="col-12">
                    <label className="form-label fw-bold text-muted small">
                      CONTRASENA {editingId && <span className="fw-normal text-muted">(dejar vacio para mantener)</span>}
                    </label>
                    <input type="password" className="form-control rounded-3 bg-light border-0 py-2" value={form.contrasena} onChange={e => setForm({ ...form, contrasena: e.target.value })} placeholder={editingId ? "********" : "Minimo 6 caracteres"} />
                  </div>
                </div>
              </div>
